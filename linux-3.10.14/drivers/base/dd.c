...
probe_failed:
    ...
    if (ret == -EPROBE_DEFER) {
        ...
        printk(
            KERN_WARNING "%s: probe of %s failed with error %d\n", drv->name, dev_name(dev), ret);
    } else {
...
static void __device_release_driver(struct device *dev) {
    ...
    if (drv) {
        ...
        if (dev->bus)
            blocking_notifier_call_chain(&dev->bus->p->bus_notifier, BUS_NOTIFY_UNBIND_DRIVER, dev);
        ...
        if (dev->bus)
            blocking_notifier_call_chain(
                &dev->bus->p->bus_notifier, BUS_NOTIFY_UNBOUND_DRIVER, dev);
    }
}
...
void driver_detach(struct device_driver *drv) {
    ...
    for (;;) {
        ...
        dev_prv = list_entry(
            drv->p->klist_devices.k_list.prev, struct device_private, knode_driver.n_node);
        dev = dev_prv->device;
        get_device(dev);
        spin_unlock(&drv->p->klist_devices.k_lock);
